...
def onboard_leadscoring(request):
    ...
    if cia_url and cia_service_key:
        ...
        try:
            ...
            # Note: Executor adapter currently only supports shopify|ciq|leadscore integrations
            # For Stripe, we may need to call Repo C directly or update executor adapter
            # For now, we'll try the executor pattern - if it fails, we'll log and continue
            # TODO: Update executor adapter regex to support "stripe" integration
