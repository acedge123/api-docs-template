...
def test_question_list():
    assert reverse('api:v1:question-list') == '/api/v1/questions/'
    assert resolve('/api/v1/questions/').view_name == 'api:v1:question-list'


def test_lead_detail(lead):
    assert reverse('api:v1:lead-detail', kwargs={'pk': str(lead.lead_id)}) == f'/api/v1/leads/{lead.lead_id}/'
