...
@pytest.mark.parametrize('model_admin', ['question_admin_and_model', 'recommendation_admin_and_model',
                                         'lead_admin_and_model', 'scoring_model_admin_and_model'])
class TestCommonRestrictedAdminMethods:
    ...
    def test_get_form_extend_help_text_of_field_to_extend_help_text(self, request, model_admin, fake_request):
        model_admin, _ = request.getfixturevalue(model_admin)
    ...
    def test_get_form_extend_help_text_of_field_to_extend_help_text_no_questions(self, request, model_admin,
                                                                                 fake_request):
        model_admin, _ = request.getfixturevalue(model_admin)
...
class TestScoringModelAdmin:
    ...
    @pytest.mark.usefixtures('questions')
    def test_get_formfield_for_foreignkey_restrict_question_options(self, scoring_model_admin_and_model, db_field_mock,
                                                                    user, user1, fake_request):
        scoring_model_admin, _ = scoring_model_admin_and_model
        question_db_field_mock = db_field_mock('question')

        scoring_model_admin.formfield_for_foreignkey(question_db_field_mock, fake_request)
        queryset = question_db_field_mock.formfield.call_args.kwargs['queryset']
    ...
    @pytest.mark.usefixtures('questions')
    def test_scoring_model_validation_field_names_are_valid(self, scoring_model_admin_and_model, fake_request, user,
                                                            formula):
        scoring_model_admin, _ = scoring_model_admin_and_model
    ...
    @pytest.mark.usefixtures('questions')
    def test_scoring_model_validation_field_names_are_not_valid(self, scoring_model_admin_and_model, fake_request,
                                                                user, formula, error_message):
        scoring_model_admin, _ = scoring_model_admin_and_model
        form_class = scoring_model_admin.get_form(fake_request)
    ...
    @pytest.mark.parametrize('start,end,deleted', [
        ('0', '1', None),
        ('1', '1', None),
        ('2', '1', 'on'),
    ])
    def test_value_range_start_less_than_or_equal_end_if_not_deleted_valid(self, scoring_model_admin_and_model,
                                                                           fake_request, start, end, deleted):
        scoring_model_admin, scoring_model = scoring_model_admin_and_model

        sm = scoring_model(id=111)

        value_range_inline = scoring_model_admin.get_inline_instances(fake_request, sm)[0]
    ...
    def test_value_range_start_less_than_or_equal_end_if_not_deleted_not_valid(self, scoring_model_admin_and_model,
                                                                               fake_request):
        scoring_model_admin, scoring_model = scoring_model_admin_and_model
        ...
        assert formset.non_form_errors() == ['Start of range must be less than or equal to End']


class TestQuestionAdmin:

    def test_min_max_values_validation_valid(self, question_admin_and_model, fake_request, question_data):
    ...
    def test_min_max_values_validation_not_valid(self, question_admin_and_model, fake_request, question_data,
                                                 min_value, max_value):
        question_admin, _ = question_admin_and_model
        form_class = question_admin.get_form(fake_request)
    ...
    @pytest.mark.parametrize('value,question_type', [
        (None, Question.OPEN),
        (None, Question.CHOICES),
        (1, Question.SLIDER),
        (0, Question.SLIDER),
    ])
    def test_min_value_is_not_required(self, question_admin_and_model, fake_request, question_data, value,
                                                    question_type):
        question_admin, _ = question_admin_and_model
        form_class = question_admin.get_form(fake_request)
    ...
    def test_min_value_is_required_for_slider_question_type(self, question_admin_and_model, fake_request, question_data):
        question_admin, _ = question_admin_and_model
        form_class = question_admin.get_form(fake_request)
    ...
    def test_max_value_is_not_required(self, question_admin_and_model, fake_request, question_data, value,
                                       question_type):
        question_admin, _ = question_admin_and_model
        form_class = question_admin.get_form(fake_request)
    ...
    def test_max_value_is_required_for_slider_question_type(self, question_admin_and_model, fake_request,
                                                            question_data):
        question_admin, _ = question_admin_and_model
        form_class = question_admin.get_form(fake_request)
    ...
    @pytest.mark.parametrize('question_type', [
        Question.OPEN, Question.SLIDER
    ])
    def test_choices_not_required_for_open_and_slider_questions_types(self, question_admin_and_model, fake_request,
                                                                      question_type):
    ...
    def test_choices_required_for_choices_question_type(self, question_admin_and_model, fake_request):
        question_admin, question_model = question_admin_and_model
    ...
    def test_choices_required_for_choices_question_type_if_all_choices_deleted(self, question_admin_and_model,
                                                                               fake_request):
        question_admin, question_model = question_admin_and_model
