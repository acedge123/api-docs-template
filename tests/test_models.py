...
from scoringengine.models import Choice, Question, Lead, Answer, Recommendation, ScoringModel, ValueRange
...
class TestRecommendation:
    ...
    def test_recommendation_raise_validation_error_rule_invalid(self, recommendation_data, rule):
    ...
    @pytest.mark.parametrize('rule,error_message', [
        ('If {Rent} not > 99', 'Rule syntax invalid "If {Rent} not >>>here>>>> 99"'),
        ('If {Rent} > not {Income}', 'Rule syntax invalid "If {Rent} > >>>here>>>not {Income}"'),
        ('If {Rent} > 99 not', 'Rule syntax invalid "If {Rent} > 99 no>>>here>>>t"'),
        ('If {Rent} +/ {Income}', re.escape('Rule syntax invalid "If {Rent} +>>>here>>>/ {Income}"')),
        ('If {Rent} + 99 -', re.escape('Rule syntax invalid "If {Rent} + 99 >>>here>>>-"')),
        ('If {Rent} + 99.99.9 > 0', re.escape('Rule syntax invalid "If {Rent} + 99.99>>>here>>>.9 > 0"')),
    ])
    def test_recommendation_raise_validation_error_rule_syntax_invalid(self, recommendation_data, rule, error_message):
        recommendation_data['rule'] = rule
...
class TestQuestion:
    ...
    def test_check_rule_question_with_no_recommendation(self, question_with_no_recommendation):
    ...
    def test_get_recommendation_dict_question_with_no_recommendation(self, question_with_no_recommendation):
...
class TestScoringModel:
    @pytest.mark.parametrize('formula,expected_result', [
    ...
    def test_scoring_model_raise_validation_error_formula_invalid(self, scoring_model_data, formula):
        scoring_model_data['formula'] = formula
    ...
    @pytest.mark.parametrize('formula,error_message', [
        ('{Rent} +/ 99', re.escape('Formula syntax invalid "{Rent} +>>>here>>>/ 99"')),
        ('{Rent} + 99.99.9', re.escape('Formula syntax invalid "{Rent} + 99.99>>>here>>>.9"')),
    ])
    def test_scoring_model_raise_validation_error_formula_syntax_invalid(self, scoring_model_data, formula,
                                                                         error_message):
        scoring_model_data['formula'] = formula
    ...
    @pytest.mark.parametrize('formula,data,expected_result', [
    ...
    @pytest.mark.parametrize('formula,answers,expected_result', [
        ('', {'test_question': -1}, 0),
        ('', {'test_question': 0}, 1.1),
